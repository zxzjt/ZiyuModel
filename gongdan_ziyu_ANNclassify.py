...
# 降维
#train_prepro,test_prepro=data_transform(train_prepro,test_prepro)

mlp_model=MLPClassifier(hidden_layer_sizes=(141,2),learning_rate="constant",alpha=0.00001,solver='lbfgs',
                        learning_rate_init=0.0001,max_iter=200)
"""
para_search=GridSearchCV(estimator=mpl_model,param_grid={'min_samples_leaf':range(1,9,1),'min_samples_split':range(9,15,2),'max_features':np.arange(0.4,1,0.2),'max_depth':range(5,12,2)},cv=5,scoring='roc_auc')
...
mlp_model.fit(train_prepro[:,:-1],train_prepro[:,-1])
predicted=mlp_model.predict(test_prepro[:,:-1])
print(metrics.confusion_matrix(train_prepro[:,-1],mlp_model.predict(train_prepro[:,:-1])))
print(metrics.confusion_matrix(test_prepro[:,-1],predicted))
print(metrics.classification_report(test_prepro[:,-1],predicted))
print("test score is %f" % mlp_model.score(test_prepro[:,:-1],test_prepro[:,-1]))
print("train score is %f" % mlp_model.score(train_prepro[:,:-1],train_prepro[:,-1]))
print(mlp_model)

#plot_learning_curve(mlp_model,'MLP',train_prepro[:,:-1],train_prepro[:,-1],cv=5)
