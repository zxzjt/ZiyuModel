...
svm_model=SVC(C=3.4,kernel='poly',gamma=0.33)
"""
para_search=GridSearchCV(estimator=svm_model,param_grid={'C':np.arange(0.2,10,0.2),'gamma':np.arange(0.01,0.5,0.02)},cv=5,scoring='roc_auc')
...
"""
svm_model.fit(train_prepro[:,:-1],train_prepro[:,-1])
predicted=svm_model.predict(test_prepro[:,:-1])
print(metrics.confusion_matrix(test_prepro[:,-1],predicted))
print(metrics.classification_report(test_prepro[:,-1],predicted))
print("test score is %f" % svm_model.score(test_prepro[:,:-1],test_prepro[:,-1]))
print("train score is %f" % svm_model.score(train_prepro[:,:-1],train_prepro[:,-1]))
print(svm_model)
