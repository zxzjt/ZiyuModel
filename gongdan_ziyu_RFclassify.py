...
rf_model=RandomForestClassifier(n_estimators=120,min_samples_leaf=1,max_depth=12,max_features=0.4,random_state=0)
"""
para_search=GridSearchCV(estimator=rf_model,param_grid={'min_samples_leaf':range(1,9,1),'min_samples_split':range(9,15,2),'max_features':np.arange(0.4,1,0.2),'max_depth':range(5,12,2)},cv=5,scoring='roc_auc')
...
rf_model.fit(train_prepro[:,:-1],train_prepro[:,-1])
predicted=rf_model.predict(test_prepro[:,:-1])
...
print("test score is %f" % rf_model.score(test_prepro[:,:-1],test_prepro[:,-1]))
print("train score is %f" % rf_model.score(train_prepro[:,:-1],train_prepro[:,-1]))
print(rf_model)
