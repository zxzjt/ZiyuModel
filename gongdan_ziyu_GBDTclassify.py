...
train=pd.concat([train_all[train_all.自愈状态=='派单'].sample(frac=0.5,axis=0,random_state=0),train_all[train_all.自愈状态=='自愈']],axis=0,join='outer')
...
gbdt_model=GradientBoostingClassifier(n_estimators=50,learning_rate=0.15,min_samples_leaf=2,max_depth=7,subsample=1,max_features=0.3)
"""
para_search=GridSearchCV(estimator=gbdt_model,param_grid={'n_estimators':range(20,100,10),'learning_rate':np.arange(0.002,0.02,0.2),'max_features':np.arange(0.3,1,0.2),'max_depth':range(7,14,1)},cv=5,scoring='roc_auc')
...
gbdt_model.fit(train_prepro[:,:-1],train_prepro[:,-1])
predicted=gbdt_model.predict(test_prepro[:,:-1])
print(metrics.confusion_matrix(train_prepro[:,-1],gbdt_model.predict(train_prepro[:,:-1])))
print(metrics.confusion_matrix(test_prepro[:,-1],predicted))
print(metrics.classification_report(test_prepro[:,-1],predicted))
print("test score is %f" % gbdt_model.score(test_prepro[:,:-1],test_prepro[:,-1]))
print("train score is %f" % gbdt_model.score(train_prepro[:,:-1],train_prepro[:,-1]))
print(gbdt_model)
#plot_learning_curve(gbdt_model,'RF',train_prepro[:,:-1],train_prepro[:,-1],cv=5)
